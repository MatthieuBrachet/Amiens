function [x, error, iter, flag,residual,s] = fouriergmres_neum3D( A, x, b, restrt, max_it, tol )
...
%                           1 = no convergence given max_it
   ...
   %r = M \ ( b-A*x );
   %FOURIER PRECOND
   nnnn=nthroot(max(size(A)),3);
   n3=max(size(A));
   re=reshape(b-A*x,nnnn,nnnn,nnnn);
   r=prec_Poiss_neum3d(re);
   r=reshape(r,n3,1,1);
   ...
   for iter = 1:max_it,                              % begin iteration
	 ...
	 AV=reshape(A*V(:,i),nnnn,nnnn,nnnn);
          w=prec_Poiss_neum3d(AV);
           w=reshape(w,n3,1,1);
